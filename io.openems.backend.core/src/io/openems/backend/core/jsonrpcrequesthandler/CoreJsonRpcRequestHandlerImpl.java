...
public class CoreJsonRpcRequestHandlerImpl extends AbstractOpenemsBackendComponent implements JsonRpcRequestHandler {
	...
	private final ChannelAggregationHandler channelAggregationHandler;
	private final ChannelHistoryAggregationHandler channelHistoryAggregationHandler;
	...
	public CoreJsonRpcRequestHandlerImpl() {
		...
		this.channelAggregationHandler = new ChannelAggregationHandler(this);
		this.channelHistoryAggregationHandler = new ChannelHistoryAggregationHandler(this);
	}
	...
	public CompletableFuture<? extends JsonrpcResponseSuccess> handleRequest(String context, User user,
		...
		case GetEdgesRequest.METHOD //
			-> this.handleGetEdgesRequest(user, request.getId(), GetEdgesRequest.from(request));
		case AggregateAndGetEdgesChannelsValuesRequest.METHOD //
			-> this.channelAggregationHandler.handleRequest(user, request.getId(),
					AggregateAndGetEdgesChannelsValuesRequest.from(request));
		case AggregateAndQueryEdgesChannelsHistoryRequest.METHOD //
			-> this.channelHistoryAggregationHandler.handleRequest(user, request.getId(),
					AggregateAndQueryEdgesChannelsHistoryRequest.from(request));
	 ...
	 * @throws OpenemsNamedException on error
	 */
	private CompletableFuture<GetEdgesResponse> handleGetEdgesRequest(User user, UUID messageId, GetEdgesRequest request)
			throws OpenemsNamedException {
		var edgeMetadata = this.metadata.getPageDevice(user, request.getPaginationOptions());
		return CompletableFuture.completedFuture(new GetEdgesResponse(messageId, edgeMetadata));
	}
