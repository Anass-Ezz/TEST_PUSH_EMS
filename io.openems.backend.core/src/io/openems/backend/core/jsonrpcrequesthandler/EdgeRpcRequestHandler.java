...
import io.openems.common.jsonrpc.request.QueryHistoricTimeseriesEnergyCostPerPeriodRequest;
import io.openems.common.jsonrpc.request.QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest;
import io.openems.common.jsonrpc.response.QueryHistoricTimeseriesEnergyResponse;
import io.openems.common.jsonrpc.response.QueryHistoricTimeseriesEnergyCostPerPeriodResponse;
import io.openems.common.jsonrpc.response.QueryHistoricTimeseriesEnergyEmissionsPerPeriodResponse;
...
public class EdgeRpcRequestHandler {
	...
	protected CompletableFuture<EdgeRpcResponse> handleRequest(User user, UUID messageId, EdgeRpcRequest edgeRpcRequest)
		...
		case QueryHistoricTimeseriesEnergyCostPerPeriodRequest.METHOD -> this.handleQueryHistoricEnergyCostPerPeriodRequest(
				edgeId, user, QueryHistoricTimeseriesEnergyCostPerPeriodRequest.from(request));

		case QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest.METHOD ->
			this.handleQueryHistoricEnergyEmissionsPerPeriodRequest(
					edgeId, user, QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest.from(request));
	...
	/**
	 * Handles a {@link QueryHistoricTimeseriesEnergyCostPerPeriodRequest}.
	 *
	 * @param edgeId  the Edge-ID
	 * @param user    the {@link User} - no specific level required
	 * @param request the {@link QueryHistoricTimeseriesEnergyCostPerPeriodRequest}
	 * @return the Future JSON-RPC Response
	 * @throws OpenemsNamedException on error
	 */
	private CompletableFuture<JsonrpcResponseSuccess> handleQueryHistoricEnergyCostPerPeriodRequest(String edgeId,
			User user, QueryHistoricTimeseriesEnergyCostPerPeriodRequest request) throws OpenemsNamedException {
		var data = this.parent.timedataManager.queryHistoricEnergyCostPerPeriod(//
				edgeId, request.getFromDate(), request.getToDate(), request.getChannels(), request.getResolution());

		return CompletableFuture
				.completedFuture(new QueryHistoricTimeseriesEnergyCostPerPeriodResponse(request.getId(), data));
	}

	/**
	 * Handles a {@link QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest}.
	 *
	 * @param edgeId  the Edge-ID
	 * @param user    the {@link User} - no specific level required
	 * @param request the {@link QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest}
	 * @return the Future JSON-RPC Response
	 * @throws OpenemsNamedException on error
	 */
	private CompletableFuture<JsonrpcResponseSuccess> handleQueryHistoricEnergyEmissionsPerPeriodRequest(String edgeId,
			User user, QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest request) throws OpenemsNamedException {
		var data = this.parent.timedataManager.queryHistoricEnergyEmissionsPerPeriod(//
				edgeId, request.getFromDate(), request.getToDate(), request.getChannels(), request.getResolution());

		return CompletableFuture
				.completedFuture(new QueryHistoricTimeseriesEnergyEmissionsPerPeriodResponse(request.getId(), data));
	}
