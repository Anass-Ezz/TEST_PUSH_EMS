...
public class TimedataManagerImpl extends AbstractOpenemsBackendComponent implements TimedataManager, MetricsConsumer {
	...
	public SortedMap<ChannelAddress, JsonElement> queryHistoricDataAggregated(//
		...
		final var resultData = this.querySingleValues(channels,
				(t, c) -> t.queryHistoricDataAggregated(edgeId, fromDate, toDate, c, aggregationFunction));

		if (resultData.isEmpty()) {
			this.logWarn(this.log, "No timedata result for 'queryHistoricDataAggregated' on Edge=" + edgeId
					+ "; FromDate=" + fromDate + "; ToDate=" + toDate + "; Channels=" + channels
					+ "; AggregationFunction=" + aggregationFunction);
			throw new OpenemsException("Unable to query historic data aggregated. Result is null");
		}
