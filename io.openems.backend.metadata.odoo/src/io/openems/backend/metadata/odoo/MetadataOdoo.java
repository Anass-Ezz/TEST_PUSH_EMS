...
public class MetadataOdoo extends AbstractMetadata implements AppCenterMetadata, AppCenterMetadata.EdgeData,
		AppCenterMetadata.UiData, Metadata, Mailer, EventHandler, DebugLoggable, TimeOfUseTariffMoroccoMetadata, Co2eScope2Metadata {
	...
	public CompletableFuture<TimeOfUseTariffMoroccoResponse> getMoroccoTariff(String edgeId,
		...
		return this.requestExecutor.submit("getMoroccoTariff", () -> {
			var result = this.odooHandler.getMoroccoTariff(edgeId, request);
			Double pricePeak = JsonUtils.getAsOptionalDouble(result, "pricePeak").orElse(null);
			Double priceStandard = JsonUtils.getAsOptionalDouble(result, "priceStandard").orElse(null);
			Double priceOffPeak = JsonUtils.getAsOptionalDouble(result, "priceOffPeak").orElse(null);
			return new TimeOfUseTariffMoroccoResponse(request.getId(), pricePeak, priceStandard, priceOffPeak);
		});
	}

	@Override
	public CompletableFuture<Co2eScope2Response> getCo2eScope2(String edgeId, Co2eScope2Request request) {
		return this.requestExecutor.submit("getCo2eScope2", () -> {
			var result = this.odooHandler.getCo2eScope2(edgeId, request);
			Double factor = JsonUtils.getAsOptionalDouble(result, "factor").orElse(null);
			return new Co2eScope2Response(request.getId(), factor);
		});
	}
