...
public class OdooHandler {
	...
	/**
	 * Gets the Morocco Tariff prices.
	 * 
	 * @param edgeId  the Edge-ID
	 ...
	 * @throws OpenemsNamedException on error
	 */
	public JsonObject getMoroccoTariff(String edgeId, TimeOfUseTariffMoroccoRequest request)
			throws OpenemsNamedException {
		var req = buildJsonObject() //
				.add("params", buildJsonObject() //
						.addProperty("edgeId", edgeId) //
						.build()) //
				.build();
		return getAsJsonObject(OdooUtils.sendAdminJsonrpcRequest(this.credentials,
				"/openems_backend/timeofusetariff_morocco", req).result);
	}

	public JsonObject getCo2eScope2(String edgeId, Co2eScope2Request request)
			throws OpenemsNamedException {
		var req = buildJsonObject() //
				.add("params", buildJsonObject() //
						.build()) //
				.build();
		return getAsJsonObject(OdooUtils.sendAdminJsonrpcRequest(this.credentials,
				"/openems_backend/co2_emission_scope2", req).result);
	}
