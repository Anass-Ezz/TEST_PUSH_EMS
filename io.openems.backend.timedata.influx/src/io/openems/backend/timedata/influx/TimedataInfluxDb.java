...
public class TimedataInfluxDb extends AbstractOpenemsBackendComponent implements Timedata, EventHandler, DebugLoggable {
	...
	@Override
	public SortedMap<ChannelAddress, JsonElement> queryHistoricDataAggregated(String edgeId, ZonedDateTime fromDate,
			ZonedDateTime toDate, Set<ChannelAddress> channels, String aggregationFunction)
			throws OpenemsNamedException {
		if (!this.timeFilter.isValid(fromDate, toDate)) {
			return null;
		}
		...
		return this.influxConnector.queryHistoricDataAggregated(Optional.of(influxEdgeId), fromDate, toDate,
				channels, this.config.measurement(), aggregationFunction);
	}
