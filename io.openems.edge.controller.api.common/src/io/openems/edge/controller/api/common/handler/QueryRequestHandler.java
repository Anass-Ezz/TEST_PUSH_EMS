...
public class QueryRequestHandler implements JsonApi {
	...
	public void buildJsonApiRoutes(JsonApiBuilder builder) {
		...
		builder.handleRequest(QueryHistoricTimeseriesEnergyCostPerPeriodRequest.METHOD, call -> {
			final var request = QueryHistoricTimeseriesEnergyCostPerPeriodRequest.from(call.getRequest());
			var data = this.getTimedata().queryHistoricEnergyCostPerPeriod(//
					null, /* ignore Edge-ID */
					request.getFromDate(), request.getToDate(), request.getChannels(), request.getResolution());
			return new QueryHistoricTimeseriesEnergyCostPerPeriodResponse(request.getId(), data);
		});

		builder.handleRequest(QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest.METHOD, call -> {
			final var request = QueryHistoricTimeseriesEnergyEmissionsPerPeriodRequest.from(call.getRequest());
			var data = this.getTimedata().queryHistoricEnergyEmissionsPerPeriod(//
					null, /* ignore Edge-ID */
					request.getFromDate(), request.getToDate(), request.getChannels(), request.getResolution());
			return new QueryHistoricTimeseriesEnergyEmissionsPerPeriodResponse(request.getId(), data);
		});
