...
public interface ElectricityMeter extends OpenemsComponent {
	...
	public enum ChannelId implements io.openems.edge.common.channel.ChannelId {
		...
		ACTIVE_CONSUMPTION_ENERGY_L3(Doc.of(OpenemsType.LONG) //
				...
				.persistencePriority(PersistencePriority.HIGH)),
		/**
		 * Active Consumption Energy Cost.
		 *
		 * <ul>
		 * <li>Interface: ElectricityMeter
		 * <li>Type: Double
		 * <li>Unit: NONE (Assumed cost unit like [MAD])
		 * </ul>
		 */
		ACTIVE_CONSUMPTION_ENERGY_COST(Doc.of(OpenemsType.DOUBLE) //
				.unit(Unit.NONE)
				.persistencePriority(PersistencePriority.HIGH)), //

		/**
		 * Active Consumption Energy Emissions.
		 *
		 * <ul>
		 * <li>Interface: ElectricityMeter
		 * <li>Type: Double
		 * <li>Unit: NONE (Technically [kg CO2e])
		 * </ul>
		 */
		ACTIVE_CONSUMPTION_ENERGY_EMISSIONS(Doc.of(OpenemsType.DOUBLE) //
				.unit(Unit.KILOGRAMS)
				.persistencePriority(PersistencePriority.HIGH)),
	...
	/**
	 * Gets the Channel for {@link ChannelId#ACTIVE_CONSUMPTION_ENERGY_COST}.
	 *
	 * @return the Channel
	 */
	public default DoubleReadChannel getActiveConsumptionEnergyCostChannel() {
		return this.channel(ChannelId.ACTIVE_CONSUMPTION_ENERGY_COST);
	}

	/**
	 * Gets the Active Consumption Energy Cost in [MAD]. See
	 * {@link ChannelId#ACTIVE_CONSUMPTION_ENERGY_COST}.
	 *
	 * @return the Channel {@link Value}
	 */
	public default Value<Double> getActiveConsumptionEnergyCost() {
		return this.getActiveConsumptionEnergyCostChannel().value();
	}

	/**
	 * Internal value for calculating the Energy Cost differences.
	 */
	public static final String ACTIVE_CONSUMPTION_ENERGY_COST_LAST_ENERGY_KEY = "ACTIVE_CONSUMPTION_ENERGY_COST_LAST_ENERGY";

	/**
	 * Calculates the continuously increasing Energy Cost based on the growing Energy
	 * and the current Tariff price. It attaches a listener to the ActiveConsumptionEnergy
	 * channel so that strictly every time the energy increments, the cost increments by
	 * (Energy Delta * Current Price).
	 *
	 * @param meter  the {@link ElectricityMeter}
	 * @param tariff the {@link io.openems.edge.timeofusetariff.morocco.api.TimeOfUseTariffMorocco} service
	 */
	public static void calculateEnergyCost(ElectricityMeter meter, TimeOfUseTariffMorocco tariff) {
		meter.getActiveConsumptionEnergyChannel().onSetNextValue(value -> {
			if (!value.isDefined() || tariff == null) {
				return;
			}
			
			long currentEnergy = value.get();
			
			Double currentPrice = tariff.getNowPrice();
			if (currentPrice == null) {
				System.out.println("[calculateEnergyCost] getNowPrice() returned null, skipping calculation.");
				return;
			}			
			// Dynamically adjust the energy to kWh based on the channel's unit
			io.openems.common.channel.Unit unit = meter.getActiveConsumptionEnergyChannel().channelDoc().getUnit();
			double currentEnergyKwh;
			
			if (unit == io.openems.common.channel.Unit.KILOWATT_HOURS) {
				currentEnergyKwh = (double) currentEnergy;
			} else {
				// Base assumption is Wh (for CUMULATED_WATT_HOURS or WATT_HOURS)
				currentEnergyKwh = currentEnergy / 1000.0;
			}

			// Directly multiply the total accumulated energy by the price
			double newTotalCost = currentEnergyKwh * currentPrice;

			DoubleReadChannel costChannel = meter.getActiveConsumptionEnergyCostChannel();
			costChannel.setNextValue(newTotalCost);
		});
	}

	/**
	 * Gets the Channel for {@link ChannelId#ACTIVE_CONSUMPTION_ENERGY_EMISSIONS}.
	 *
	 * @return the Channel
	 */
	public default DoubleReadChannel getActiveConsumptionEnergyEmissionsChannel() {
		return this.channel(ChannelId.ACTIVE_CONSUMPTION_ENERGY_EMISSIONS);
	}

	/**
	 * Gets the Active Consumption Energy Emissions [kg CO2e]. See
	 * {@link ChannelId#ACTIVE_CONSUMPTION_ENERGY_EMISSIONS}.
	 *
	 * @return the Channel {@link Value}
	 */
	public default Value<Double> getActiveConsumptionEnergyEmissions() {
		return this.getActiveConsumptionEnergyEmissionsChannel().value();
	}

	/**
	 * Calculates the continuously increasing Energy Emissions based on the growing Energy
	 * and the current Co2e factor. It attaches a listener to the ActiveConsumptionEnergy
	 * channel so that strictly every time the energy increments, the factor is multipled by
	 * (Energy * Factor).
	 *
	 * @param meter  the {@link ElectricityMeter}
	 * @param emission the {@link io.openems.edge.co2e.scope2.Co2eScope2} service
	 */
	public static void calculateEnergyEmissions(ElectricityMeter meter, Co2eScope2 emission) {
		meter.getActiveConsumptionEnergyChannel().onSetNextValue(value -> {
			if (!value.isDefined() || emission == null) {
				return;
			}
			
			long currentEnergy = value.get();
			Double currentFactor = emission.getNowFactor();
			...
			// Directly multiply the total accumulated energy by the emission factor
			double newTotalEmissions = currentEnergyKwh * currentFactor;

			DoubleReadChannel emissionsChannel = meter.getActiveConsumptionEnergyEmissionsChannel();
			emissionsChannel.setNextValue(newTotalEmissions);
		});
	}
