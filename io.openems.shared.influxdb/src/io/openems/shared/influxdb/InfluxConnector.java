...
public class InfluxConnector {
	...
	/**
	 * Queries historic data aggregated to a single value per channel using the
	 ...
	 * @param channels            the Channels to query
	 * @param measurement         the measurement
	 * @param aggregationFunction the aggregation function (e.g. "mean", "sum")
	 * @return a map between ChannelAddress and aggregated value
	 * @throws OpenemsNamedException on error
	 */
	public SortedMap<ChannelAddress, JsonElement> queryHistoricDataAggregated(Optional<Integer> influxEdgeId,
			ZonedDateTime fromDate, ZonedDateTime toDate, Set<ChannelAddress> channels, String measurement,
			String aggregationFunction) throws OpenemsNamedException {
		// handle empty call
		if (channels.isEmpty()) {
			return emptySortedMap();
		}

		return this.queryProxy.queryHistoricDataAggregated(this.getInfluxConnection(), this.bucket, measurement,
				influxEdgeId, fromDate, toDate, channels, aggregationFunction);
	}
